# Original source: https://github.com/zhengjie9510/google-map-downloader/blob/master/downloader_1.2.py
# -*- coding: utf-8 -*
'''
...
def saveTiff(r, g, b, gt, filePath, debug=False):
    fname_out = filePath
...
def get_urls(x1, y1, x2, y2, z, source, style, debug=False):
    pos1x, pos1y = wgs_to_tile(x1, y1, z)
    pos2x, pos2y = wgs_to_tile(x2, y2, z)
    lenx = pos2x - pos1x + 1
    leny = pos2y - pos1y + 1
...
# ---------------------------------------------------------
def merge_tiles(datas, x1, y1, x2, y2, z, debug=False):
    pos1x, pos1y = wgs_to_tile(x1, y1, z)
    pos2x, pos2y = wgs_to_tile(x2, y2, z)
    lenx = pos2x - pos1x + 1
    leny = pos2y - pos1y + 1
...
# ---------------------------------------------------------
def main(left, top, right, bottom, zoom, filePath, style='s', server="Google China", debug=False):
    """
    ...
    server : Google China (default) or Google
    """
    # ---------------------------------------------------------
    # Get the urls of all tiles in the extent
    urls = get_urls(left, top, right, bottom, zoom, server, style, debug=debug)
    ...
    saveTiff(r, g, b, gt, filePath, debug=debug)
...
if __name__ == '__main__':
    ...
    main(100.361, 38.866, 100.386, 38.839, 18, r'test.tif', server="Google")
